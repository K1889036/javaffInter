...
public class NullFilter implements Filter
{
	...
	public List<Action> getActions(State S)
	{
		...
		//System.out.println("gerer "+actionsFromS);											// actions in S
		...
		while (ait.hasNext())
		{
			...
			if (a.isApplicable(S))
				//System.out.println(a.getPreconditions());
				ns.add(a); // Check they are applicable (will check
							// numeric/temporal constraints)
		}
		return ns;
	}
	public List<Action> getNonApplicableActions(State S){
		
		Set<Action> actionFromS = S.getActions();
		List fa = new ArrayList();
		Iterator it = actionFromS.iterator();
		while(it.hasNext()) {
